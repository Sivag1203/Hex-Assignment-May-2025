...
function AssetRequests({ onClose }) {
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get(
        "http://localhost:8080/api/asset-requests/all",
        {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      if (newStatus === "approved") {
        await axios.put(
          `http://localhost:8080/api/asset-requests/approve/${id}`,
          {},
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } else if (newStatus === "rejected") {
        await axios.delete(
          `http://localhost:8080/api/asset-requests/reject/${id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      }
      fetchRequests(); // Refresh data after status change
    } catch (err) {
      console.error("Failed to update asset request status", err);
    }
  };
